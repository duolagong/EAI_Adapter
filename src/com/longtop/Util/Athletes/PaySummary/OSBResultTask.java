...
public class OSBResultTask implements Task{

    private static Logger logger = Logger.getLogger(ATOMResultTask.class);
    @Test
    @Override
    public void executeTask() {
        logger.info("OSBResultTask-执行的任务:"+ DateUtil.getDate("yyyy-MM-dd HH:mm:dd")+"Start!");
        Session session=null;
        try {
            HibernateContext hiContext = LoadEngine.getDBContext("SQL");
            String osbSql="select pay.ordernum , pay.version , pay.subbranchid , pay.agentid from bnk_pay_infor pay " +
                    "where pay.payprocess='1' and pay.txcode='1000' and pay.agentid<>'Z01' and pay.agentid<>'W01'" +
                    " Union all " +
                    "select pay.ordernum , '21' as version , pay.subbranchid , pay.agentid from bnk_pay_infor pay " +
                    "where pay.payprocess='1' and pay.txcode='2000' and pay.agentid<>'Z01' and pay.agentid<>'W01' and pay.nbkcode is not null";
            session = hiContext.getSession();
            SQLQuery sqlQuery = session.createSQLQuery(osbSql);
            ...
            logger.info("OSBResultTask-执行的任务:"+DateUtil.getDate("yyyy-MM-dd HH:mm:dd")+"End!");
        }catch (Exception e){
            e.printStackTrace();
        }finally {
            session.close();
